...
async def get_public_laws_by_congress(
    ...
    law_type: Annotated[
        str,
        Field(description="Law type: 'public' or 'private' (leave empty for both)"),
    ] = "",
    ...
    Uses the GovInfo search API to find laws enacted by the specified
    Congress.

    Args:
        congress: The Congress number to search.
        law_type: Optional 'public' or 'private' law type filter.
        law_number: Optional specific law number filter.
        start_date: Optional start date filter (YYYY-MM-DD).
        end_date: Optional end date filter (YYYY-MM-DD).
        page_size: Number of results per page.
        offset_mark: Pagination offset mark for the next page.
    ...
    """
    await log_info(ctx, f"Searching laws from Congress {congress}")
    _require_api_key()
    ...
    data = await _search_govinfo(request_body, ctx, "Congress laws search")
    await log_info(ctx, f"Found {data.get('count', 0)} laws from Congress {congress}")
    return data
...
async def get_statutes_at_large(
    ...
    Uses the GovInfo search API to find statutes from a specific volume of
    the Statutes at Large.

    Args:
        volume: The Statutes at Large volume number to search.
        page: Optional specific page number filter.
    ...
    Returns:
        dict: Statutes at Large packages and metadata.
    ...
    """
    await log_info(ctx, f"Searching Statutes at Large Volume {volume}")
    _require_api_key()
    ...
    data = await _search_govinfo(request_body, ctx, "Statutes at Large search")
    await log_info(ctx, f"Found {data.get('count', 0)} statutes in Volume {volume}")
    return data
